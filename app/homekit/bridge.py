...
def _fetch_devices(api_url: str, token: str) -> list[dict]:
    ...
    if not data.get("success"):
        raise RuntimeError(f"获取设备列表失败: {data.get('error')}")
...
def start_homekit_bridge(
    ...
    persist_file = persist_file or os.path.join(
        os.path.dirname(__file__), "..", "..", _PERSIST_FILE
    )

    if not token:
        raise ValueError("未设置 MIJIA_TOKEN 环境变量或传入 token 参数")
