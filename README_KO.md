...
- **BLE Bluetooth 센서** — PC Bluetooth로 Xiaomi BLE 온습도계 직접 연결, 로컬 실시간 데이터 수집, 자동화 연동
...
| BLE 스캔 | [bleak](https://github.com/hbldh/bleak) >= 0.22 |
...
│   ├── cli/                 # Click CLI 명령
│   ├── homekit/             # HomeKit 브릿지
│   └── ble/                 # BLE Bluetooth 센서 데몬 (독립 프로세스)
...
| BLE 센서 | `/api/ble/` | BLE 기기 등록, 데이터 보고, 이력 조회 |
...
| `list_homes` | 홈 목록 |
| `get_home` | 홈 상세 확인 |
| `list_ble_devices` | BLE 센서 기기 목록 |
| `get_ble_sensor` | BLE 센서 최신 데이터 |
| `get_ble_readings` | BLE 센서 이력 조회 |
...
## BLE Bluetooth 센서 (로컬 데이터 수집)

PC Bluetooth로 Xiaomi BLE 온습도계에 직접 연결. 추가 게이트웨이 하드웨어 불필요. 데이터 표시, 이력 조회, 자동화 연동 지원.

### 아키텍처

```
BLE 온도계  ─BLE broadcast→  BLE Scanner (독립)  ─HTTP POST→  Flask API  →  DB
...
> Bluetooth 기능이 있는 PC 필요 (Windows 10/11 내장 지원).
...
`MIJIA_TOKEN`이 설정되어 있는지 확인 (MCP 서버/ HomeKit 브릿지와 동일).

### 사용 단계

```bash
# 1. 근처 BLE 기기 스캔하여 MAC 주소 확인
mijia-control ble scan

# 2. BLE 기기 등록 (클라우드에서 암호화 키 자동 획득)
mijia-control ble register --did "blt.3.xxxxx" --mac "A4:C1:38:XX:XX:XX"

# 3. BLE 데몬 시작 (웹 서비스 실행 필요)
python run.py           # 터미널 1
python -m app.ble       # 터미널 2
...
| 기기 | 모델 | 데이터 |
|------|------|--------|
| 미지아 온습도 센서 미니 | LYWSD03MMC | 온도, 습도, 배터리 |
| 미지아 온습도 센서 (원형) | LYWSDCGQ | 온도, 습도, 배터리 |
| 미지아 온습도 센서 (신제품) | MJWSD05MMC | 온도, 습도, 배터리 |

📖 **상세 문서**: [docs/ble.md](docs/ble.md) — 아키텍처, 설정, 디버깅, 문제 해결, 새 기기 확장 방법.
...
```bash
mijia-control ble scan                          # 근처 BLE 기기 스캔
mijia-control ble register --did <did> --mac <mac>  # BLE 기기 등록
mijia-control ble list                          # BLE 기기 및 최신 읽기
mijia-control ble readings <did> --hours 24     # 이력 데이터 조회
```
